                    ...
                    for (const auto& neighbour : _params.maze.get_neighbours(curr.m_index)) {
                        ...
                        if constexpr (std::is_same_v<std::decay_t<decltype(_params.maze)>, mazes::graph<index_t, scalar_t>>) {
                            ...
                            if (!closed.contains(n)) {
                                ...
                                open.emplace(n, curr.m_gScore + nDistance, _params.h(nCoord, _params.end) * _params.weight, &buf.emplace(std::move(curr)));
                            }
                        }
                        else {
                    ...
                    for (const auto& neighbour : _params.maze.get_neighbours(curr.m_index)) {
                        ...
                        if constexpr (std::is_same_v<std::decay_t<decltype(_params.maze)>, mazes::graph<index_t, scalar_t>>) {
                            ...
                            if (!closed.contains(n)) {
                                 closed.allocate(n, capacity, _params.maze.count());
                                 closed.emplace(n);
                                   open.emplace(n, curr.m_gScore + nDistance, _params.h(nCoord, _params.end) * _params.weight, &buf.emplace(std::move(curr))); // Note: 'current' is now moved!
                            }
                        }
                        else {
